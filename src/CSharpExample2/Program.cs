...
Analyzer standardAnalyzer = new StandardAnalyzer(_luceneVersion);
...
var parser = new QueryParser(_luceneVersion, f: "title", standardAnalyzer);
var query = parser.Parse("open source");
var topDocs = searcher.Search(query, n: 3); // Indicate we want the first 3 results.

Console.WriteLine($"Matching results: {topDocs.TotalHits}");

for (var ixDoc = 0; ixDoc < topDocs.TotalHits; ixDoc++)
{
    // Read back a doc from the results.
    var resultDoc = searcher.Doc(topDocs.ScoreDocs[ixDoc].Doc);

    var domain = resultDoc.Get("domain");
    Console.WriteLine($"Domain of result {ixDoc + 1}: {domain}");
}